...
### Added
- **任务执行实时状态反馈** ✨：
  - 首页新增执行状态卡片（任务执行中/正在提交）
  - 显示当前执行阶段（分析/启动/导航/操作/验证）
  - 进度条实时更新
  - 阶段图标和文字说明
...
- **首页任务卡片交互优化** ✨：
  ...
  - 新增风险等级指示器（⚠️高风险/🚫禁止）
  - 支持长按菜单操作（暂停/启用/删除）
  - SmartCard 组件新增 onLongClick 支持

## [0.9.5] — 2026-05-29

### Improved
- **自然语言解析增强** ✨：
  - 应用数据库扩展：新增 50+ 应用（钉钉、飞书、知乎、爱奇艺、携程等）
  - 英文应用名支持：wechat/weixin → 微信，douyin/tiktok → 抖音等
  - 时间模式识别增强：支持 am/pm 格式、every morning/day 等英文表达
  - 风险关键词扩展：新增投资理财、密码修改等禁止操作
  - 任务名称生成优化：更智能的名称提取和生成逻辑

## [0.9.4] — 2026-05-29

### Fixed
- **首页失败任务点击无反应** 🟡：首页"需要处理"区域的失败任务卡片点击后没有导航到 Trace Explainer。添加 `onNavigateToTrace` 回调，点击后跳转到失败诊断页面
